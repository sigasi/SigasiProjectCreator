...
The project and all tests run under Python 3. You may need to add the
`src` and `src/SigasiProjectCreator/DotF` directories (the latter for
`.f` files only) to your `PYTHONPATH` environment, e.g.

* Linux/bash: `export PYTHONPATH=/home/mydir/SigasiProjectCreator/src:/home/mydir/SigasiProjectCreator/src/SigasiProjectCreator/DotF`
* Windows: `set PYTHONPATH=C:\work\SigasiProjectCreator\src;C:\work\SigasiProjectCreator\src\SigasiProjectCreator\DotF`

For `.f` file parsing, the Antlr4 python runtime must be installed:
